...
class RLManager:
    """Reinforcement Learning Manager - orkestrator for MuZero episode loop.

    Implementerer EPISODE_LOOP pseudokoden fra oppgaven.
    Koordinerer mellom spill, MCTS, og nettverkstrening.
    """
    ...
    def run(self):
        ...
        for episode in range(self.config.num_episodes):
            ...
            for k in range(self.config.steps_per_episode):
                ...
                if episode % 50 == 0 and k < 3:
                    ...
                    # Log visit distribution fra MCTS tree
            ...
            # Debug: vis value range
            if episode % 10 == 0:
                print(f"  Value range: {min(e[1] for e in episode_data):.2f} to {max(e[1] for e in episode_data):.2f}")
            
            # Debug: vis values og rewards statistikk
            ...
            # Beregn policy entropy (indikerer utforsking vs konvergens)
            avg_entropy = np.mean([-np.sum(p * np.log(p + 1e-8)) for p in policies])
            policy_max_probs = [np.max(p) for p in policies]
            avg_max_prob = np.mean(policy_max_probs)
            ...
            collections = sum(1 for r in rewards if r > 0.5)  # +1 rewards = collections
            ...
            # Buffer statistikk
            if episode % 50 == 0 and len(self.buffer) > 0:
                all_rewards = []
                all_values = []
                ...
                # Sjekk for skjev reward-distribusjon
            ...
            avg_loss = 0.0
            if episode > 0 and episode % self.config.training_interval == 0:
            ...
            if episode > 0 and episode % 25 == 0:
                ...
                if diag:
                    ...
                    # VIKTIG: Sjekk for representasjonskollaps
            ...
            # Logg fremgang
            if episode % 10 == 0 or episode == self.config.num_episodes - 1:
        ...
        if final_diag:
            ...
            # Analyse av hva nettverket lærte
        ...
        return psi_params, reward_history, loss_history

    def _gather_lookback_states(self, state_history, k):
        """Samle q+1 lookback states, med padding for tidlige steg.

        Args:
            state_history: liste av state arrays
            k: nåværende steg-indeks

        Returns:
            liste av q+1 state arrays
        """
    ...
    def play_episode(self, psi_params, render=False):
        """Spill en episode med trent policy (NNr + NNp, uten MCTS).

        Brukes for evaluering etter trening.

        Args:
            psi_params: trente nettverksparametre
            render: vis spilltilstanden
        ...
        for k in range(self.config.steps_per_episode):
            ...
            # Bruk bare NNp for policy (uten MCTS)
            policy, value = self.trinet.prediction(psi_params, sigma)
            policy = np.array(policy)

            # Velg handling med høy sannsynlighet (nesten greedy)
